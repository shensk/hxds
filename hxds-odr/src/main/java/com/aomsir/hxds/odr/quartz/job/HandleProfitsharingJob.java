...
public class HandleProfitsharingJob extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        //查询分账记录ID、分账金额
        map = this.profitsharingDao.searchDriverIncome(uuid);
        ...
        try {
            ...
            //生成数字签名
            String sign = WXPayUtil.generateSignature(param, this.myWXPayConfig.getKey(), WXPayConstants.SignType.HMACSHA256);
            //添加数字签名
            param.put("sign", sign);
            ...
            //验证响应的数字签名
            if (WXPayUtil.isSignatureValid(response, this.myWXPayConfig.getKey(), WXPayConstants.SignType.HMACSHA256)) {
                //从响应中提取数据
                ...
                if ("SUCCESS".equals(resultCode) && "SUCCESS".equals(returnCode)) {
                    ...
                    if ("FINISHED".equals(status)) {
                        //更新分账状态
                        int rows = this.profitsharingDao.updateProfitsharingStatus(profitsharingId);
                        if (rows != 1) {
