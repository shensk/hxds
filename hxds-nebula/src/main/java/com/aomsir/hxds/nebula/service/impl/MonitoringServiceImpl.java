...
public class MonitoringServiceImpl implements MonitoringService {
    ...
    public void monitoring(MultipartFile file, String name, String text) {
        //把录音文件上传到Minio
        try {
            MinioClient client = new MinioClient.Builder().endpoint(this.endpoint).credentials(this.accessKey, this.secretKey).build();
            client.putObject(
        ...
        entity.setText(text);
        //把文稿保存到HBase
        int rows = this.orderVoiceTextDao.insert(entity);
        if (rows != 1) {
